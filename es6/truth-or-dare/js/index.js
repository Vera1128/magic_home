...
var vue = new Vue({
    ...
    listAll: [
      {
        ...
        htmlDOM: '<div class="avatar">' +
                 ...
                 '</div>' +
                 '<div class="text-container">Hi，yangyang你准备好和我玩真心话大冒险了吗？' +
                 '</div>'
      },
      {
        isSelf: true,
        htmlDOM: '<div class="text-container">准备好啦，开始吧~' +
                 '</div>' +
                 '<div class="avatar">' +
      ...
      },
      {
        ...
        htmlDOM: '<div class="avatar">' +
                 '<img class="head-img" src="img/head-img1.jpeg"/>' +
                 '</div>' +
                 '<div class="text-container">你想要问我什么问题呢？我都可以回答你的哦~' +
                 '</div>'
      },
      {
        isSelf: true,
        htmlDOM: '<div class="text-container">喜欢女生矜持些还是活泼些？<br>·矜持些<br>·活泼些<br>·都喜欢<br>' +
                 '</div>' +
                 '<div class="avatar">' +
                 '<img class="head-img" src="img/head-img2.jpeg"/>' +
                 '</div>'
      ...
      },
      {
        ...
        htmlDOM:  '<div class="avatar">' +
                  '<img class="head-img" src="img/head-img1.jpeg"/>' +
                  '</div>' +
                  '<div class="text-container">你能接受暧昧的关系么？' +
                  '</div>'
      },
      {
        isSelf: false,
  ...
  methods: {
    clickBtn () {
      ...
      if (this.gameIndex == 0) {
        ...
        this.timer1 = setTimeout(() => {
          this.state.isLockedBtn = false;
          this.list.push(this.listAll[2]);
          Vue.nextTick(() => {
        ...
        this.timer2 = setTimeout(() => {
          this.list.push(this.listAll[4]);
          Vue.nextTick(() => {
            document.getElementsByClassName('chat-list')[0].scrollTop = 1000;
          })
          this.btnText = '正在等待对方向你提问';
          this.timer3 = setTimeout(() => {
            // 进入提问2阶段
            this.gameIndex = 2;
            this.state.isLockedBtn = false;
            this.list.push(this.listAll[5]);
            Vue.nextTick(() => {
              document.getElementsByClassName('chat-list')[0].scrollTop = 1000;
            })
            this.btnText = '发送真心话';
            this.state.showAnswerOne = true;
          }, 2000);
        }, 2000);
      }
        ...
        this.timer4 = setTimeout(() => {
          this.list.push(this.listAll[6]);
          Vue.nextTick(() => {
            document.getElementsByClassName('chat-list')[0].scrollTop = 1000;
          })
          this.btnText = '进入玩骰子大冒险游戏环节';
          this.list.push(this.listAll[7]);
          this.timer5 = setTimeout(() => {
            this.list.push(this.listAll[8]);
            Vue.nextTick(() => {
    ...
    answerSelect (quizIndex, answerIndex) {
      if (quizIndex == 1){
