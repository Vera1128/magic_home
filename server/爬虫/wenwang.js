...
// fs.readFile('./wenwang21.json',function(err,data){
//   if(err){
//     return console.error(err);
//   }
//   var dataTarget = data.toString();//将二进制的数据转换为字符串
...
//       return console.error(err);
//     }
//     var dataAll = data.toString();//将二进制的数据转换为字符串
//     dataAll = JSON.parse(dataAll);//将字符串转换为json对象
//     dataAll[type_value] = dataTarget[type_value];
//     var str = JSON.stringify(dataAll, null, 4);//因为nodejs的写入文件只认识字符串或者二进制数，所以把json对象转换成字符串重新写入json文件中
//     fs.writeFile('./wenwangAll.json',str,function(err){
//       if(err){
//         console.error(err);
//       }
